...
public class AdminBean {
	...
	* @throws ru.carabi.server.CarabiException если текущий пользователь не может выдать данное право или если (при autocreate == false) права с таким названием не существует
	...
	public void assignPermissionForUser(UserLogon logon, CarabiUser user, String permissionSysname, boolean isAssigned, boolean autocreate) throws CarabiException {
		...
		if (autocreate) {
			permission = autocreatePermission(logon, permissionSysname);
		} else {
	...
	private Permission autocreatePermission(UserLogon logon, String permissionSysname) throws CarabiException {
		logon.assertAllowed("ADMINISTRATING-PERMISSIONS-EDIT");
		Permission permission = EntityManagerTool.findBySysnameOrCreate(em, Permission.class, permissionSysname);
