...
public class CarabiDocumentBean {
	...
	private UserLogon ul; // I don't think we need this. Pls, consider. Reason: must not be a field
	private int typeId;
	private int status;
	
	public ArrayList<ArrayList<?>> getDocsTableHeader(UserLogon ul, Long kindId) throws CarabiException {
	 ...
	 */
	public ArrayList<CarabiDocProperty> getPropertiesList(UserLogon ul, int typeIdInput, long documentId)  throws CarabiException {
		...
		try {
			if (documentId > 0) {//если документ задан -- берём из него тип и статус, сверяем тип.
				status = getDocumentStatus(ul, documentId);
				typeId = getDockindID(ul, documentId);
				...
				typeId = typeIdInput;
				status = getDoceventkindId(ul, "created");
			}
			...
			queryStorage.runQuery(ul, Q_DOC_F_GET_PROPERTIES_LIST, parameters, -Integer.MAX_VALUE);
	 ...
	 * @throws SQLException 
	 */
	public String getDockindName(UserLogon ul, long documentId) throws CarabiException, SQLException {
		String dockindName = (String)queryStorage.runSimpleQuery(ul, "getDockindName", ""+documentId);
		return dockindName;
	}
	 ...
	 * @throws SQLException 
	 */
	public int getDockindID(UserLogon ul, long documentId) throws CarabiException, SQLException {
		BigDecimal dockindID = (BigDecimal)queryStorage.runSimpleQuery(ul, "getDockindID", ""+documentId);
	 ...
	 * @throws SQLException 
	 */
	public int getDocumentStatus(UserLogon ul, long documentId) throws CarabiException, SQLException {
		BigDecimal statusID = (BigDecimal)queryStorage.runSimpleQuery(ul, "DocfGetStatus", ""+documentId);
	 ...
	 */
	public int getDoceventkindId(UserLogon ul, String eventName) throws CarabiException, SQLException {
		BigDecimal statusID = (BigDecimal)queryStorage.runSimpleQuery(ul, "QDocFGetDocEventKindID", eventName);
		return statusID.intValue();
	}
	...
	public int docFGetPermissions(int dockindId, Integer doceventkindId, Integer docpropId) throws CarabiException, SQLException {
		...
		queryStorage.runQuery(ul, "docfGetPermissions", parameters, -1);
	...
	public int isDocKindCanLink(int parentKindId, int childKindId) throws CarabiException, SQLException { 
		...
		queryStorage.runQuery(ul, "qDocFIsDocKindCanLink", parameters, -1);
	...
	public class CarabiDocProperty {
		...
		private CarabiDocProperty(LinkedHashMap<String, ?> docPropertyData) throws CarabiException, SQLException {
			...
			if (docpropKind == 9) {//прямая ссылка
				...
				queryStorage.runQuery(ul, "qDocFNavGetRefInfo", parameters, -Integer.MAX_VALUE);
