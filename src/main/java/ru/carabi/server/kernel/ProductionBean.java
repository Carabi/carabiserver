...
public class ProductionBean {
	...
	public SoftwareProduct findProduct(String productSysname) {
		return EntityManagerTool.findBySysname(em, SoftwareProduct.class, productSysname);
	}
	 ...
	 * Если продукт не существует, то при autocreate == true он создаётся
	 * (текущий пользователь должен иметь право ADMINISTRATING-PRODUCTS-EDIT для этого),
	 ...
	 * @param autocreate создать продукт с кодовым названием productSysname, если его нет
	 * @return право на использование продукта, выданное или изъятое у пользователя
	 * @throws ru.carabi.server.CarabiException
	 */
	public Permission allowForUser(UserLogon logon, String productSysname, String login, boolean isAllowed, boolean autocreate) throws CarabiException {
		...
		if (product == null) {
			if (autocreate) {
				...
				product.setName(productSysname);
				product.setSysname(productSysname);
				if (productSysname.contains("-")) {
					String parentSysname = productSysname.substring(0, productSysname.lastIndexOf("-"));
					SoftwareProduct parentProduct = EntityManagerTool.findBySysname(em, SoftwareProduct.class, parentSysname);
					if (parentProduct != null) {
						product.setParentProductId(parentProduct.getId());
					}
				}
				product.setDescription("Autogenerated by ProductionBean.allowForUser");
				product.setVisible(false);
				product = em.merge(product);
				em.flush();
			} else {
				throw new CarabiException("Product " + productSysname + " not found");
			}
		}
		return allowForUser(logon, product, user, isAllowed, autocreate);
	}
	 ...
	 * @param isAllowed true -- дать доступ, false -- снять доступ
	 * @param user
	 * @param autocreate создать право использования продукта, если его нет
	 * @return право на использование продукта, выданное или изъятое у пользователя
	 * @throws ru.carabi.server.CarabiException
	 */
	public Permission allowForUser(UserLogon logon, SoftwareProduct product, CarabiUser user, boolean isAllowed, boolean autocreate) throws CarabiException {
		logger.log(Level.INFO, "allowForUser called with product {0}, user {1}", new Object[]{product.getSysname(), user.getLogin()});
		Permission permissionToUse = product.getPermissionToUse();
		if (permissionToUse == null) {
			if (autocreate) {
				logon.assertAllowed("ADMINISTRATING-PERMISSIONS-EDIT");
				permissionToUse = new Permission();
				permissionToUse.setName("Using " + product.getName());
				permissionToUse.setSysname(product.getSysname());
				permissionToUse.setDescription(permissionToUse.getName() + " (autogenerated by ProductionBean.allowForUser)");
				permissionToUse = em.merge(permissionToUse);
				em.flush();
				product.setPermissionToUse(permissionToUse);
				product = em.merge(product);
			}
			else {
				return null;
	 ...
	 * @param logon сессия текущего пользователя
	 * @param productSysname
	 * @param login
	 * @param granted
	 * @param autocreate
	 */
	public void grantForUser(UserLogon logon, String productSysname, String login, boolean granted, boolean autocreate) throws CarabiException {
		Permission permissionToUse = allowForUser(logon, productSysname, login, granted, autocreate);
		//Ищем право на выдачу права пользования продуктом
		String sql = "select permission_to_assign.sysname from\n" +
				"  software_production, user_permission permission_to_use, user_permission permission_to_assign\n" +
				"where permission_to_use.permission_id = software_production.permission_to_use\n" +
				"and   permission_to_use.permission_to_assign = permission_to_assign.permission_id";
		Query getGrantPermission = em.createNativeQuery(sql);
		List resultList = getGrantPermission.getResultList();
		String grantPermissionSysname = null;
		boolean grantPermissionAutocreating = false;
		if (resultList.isEmpty()) {
			if (!autocreate) {
				throw new CarabiException("Product " + productSysname + " does not have grant permission");
			}
			grantPermissionSysname = productSysname + "_assign";
			grantPermissionAutocreating = true;
		} else {
			grantPermissionSysname = (String)resultList.get(0);
		}
		admin.assignPermissionForUser(logon, usersPercistence.findUser(login), grantPermissionSysname, granted, grantPermissionAutocreating);
		Permission grantPermission = EntityManagerTool.findBySysname(em, Permission.class, grantPermissionSysname);
		permissionToUse.setPermissionToAssignId(grantPermission.getId());
		em.merge(permissionToUse);
	}
	...
	public boolean allowedForUser(CarabiUser user, Collection<Permission> userPermissions, Publication publication) {
		...
		if (userPermissions == null) {
			userPermissions = usersPercistence.getUserPermissions(user, (Permission)null);
		}
